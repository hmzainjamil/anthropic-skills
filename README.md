...
> Official Anthropic skill patterns for Claude Code
...
|---------|-------------|--------|
| Core — Official foundation for anthropic workflows | Core foundation — the primary abstraction this repo builds on | [docs](#) |
| Execution — task decomposition and anthropic routing | Execution layer — how tasks get decomposed and routed | [docs](#) |
| Integration — Claude Code + skills connectivity | Integration layer — connecting to external tools and APIs | [docs](#) |
...
```bash
# Quick start
python3 main.py --task "anthropic task here"

# Power user shortcut
python3 main.py --anthropic --fast
...
Use anthropic-skills to automate official anthropic skill patterns for claude code. Perfect for agencies, freelancers, and AI-first teams running anthropic workflows at scale.
...
```bash
# Add to CLAUDE.md
echo "Auto-activate: anthropic, skills" >> ~/.claude/CLAUDE.md

# Or load skill directly
/load-skill anthropic-skills
```
...
{
  "hooks": {
    "PostToolUse": [{"matcher": "anthropic", "command": "python3 ~/repos/anthropic-skills/main.py"}]
  }
}
```
...
```bash
# Fork and clone
gh repo fork hmzainjamil/anthropic-skills --clone
cd anthropic-skills
